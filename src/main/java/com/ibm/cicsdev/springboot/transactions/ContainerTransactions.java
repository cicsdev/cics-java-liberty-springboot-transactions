...
public class ContainerTransactions { 
	@Transactional	public void exampleTransactional(String... text) 
	{     // Set up our TSQ to be ready to write
		TSQ targetQueue = new TSQ(); targetQueue.setName("EXAMPLE"); 
		for (String texts : text) {    
			if (texts.contains("error")) {    
				throw new RuntimeException("Texts contains error so rollback");    
			}    
			try { targetQueue.writeString("Writing " + texts); 
			} 
			catch (InvalidRequestException | IOErrorException | LengthErrorException | InvalidSystemIdException | ISCInvalidRequestException | ItemErrorException | NotAuthorisedException e) {
				e.printStackTrace(); 
				throw new RuntimeException("TSQ Write Failed"); 
				} 
			} 
		}
	}
